...
class SequentialTrainer:
    ...
    def train_tokenizer_phase(self):
        ...
        best_tok_dir = self.config.tokenizer_best_model_path
        if os.path.exists(best_tok_dir) and len(os.listdir(best_tok_dir)) > 0:
            if self.rank == 0:
                print(f"\n🔄 [CHECKPOINT AKTİF] Daha önce başarıyla eğitilen model kasanızdan ({best_tok_dir}) yükleniyor...")
                print("💡 [BİLGİ] Ekranda Epoch 1 yazsa dahi, matematiksel olarak doğrudan 1. Epoktaki ustalığın üzerine inşa ediliyor!")
            logger.info(f"Resuming tokenizer from checkpoint: {best_tok_dir}")
            tokenizer = KronosTokenizer.from_pretrained(best_tok_dir)
        elif getattr(self.config, 'pre_trained_tokenizer', True):
            logger.info("Loading pretrained tokenizer...")
            if self.rank == 0:
                print("Loading pretrained tokenizer from HuggingFace...")
    ...
    def train_basemodel_phase(self):
        ...
        best_pred_dir = self.config.basemodel_best_model_path
        if os.path.exists(best_pred_dir) and len(os.listdir(best_pred_dir)) > 0:
            if self.rank == 0:
                print(f"\n🔄 [PREDICTOR CHECKPOINT AKTİF] Eğitilmiş ana tahmici ({best_pred_dir}) hafızaya alınıyor... Kaldığımız rekor üzerinden gidiliyor!")
            logger.info(f"Resuming predictor from checkpoint: {best_pred_dir}")
            model = Kronos.from_pretrained(best_pred_dir)
        elif getattr(self.config, 'pre_trained_predictor', True):
            logger.info("Loading pretrained predictor...")
            if self.rank == 0:
                print("Loading pretrained predictor from HuggingFace...")
        ...
        try:
            ...
            if self.rank == 0:
                print("\n🚨 [ACİL DURUM] Kullanıcı eğitimi (Ctrl+C) ile kesti! O ana kadarki ağırlıklar (Weights) acilen kaydediliyor...")
                model_save_path = os.path.join(self.config.basemodel_save_path, "best_model")
                os.makedirs(model_save_path, exist_ok=True)
                (model.module if hasattr(model, 'module') else model).save_pretrained(model_save_path)
                print(f"✅ Kurtarma Tamamlandı: {model_save_path}")
            raise
