...
def train_tokenizer(model, device, config, save_dir, logger):
    ...
    start_epoch = 0
    epoch_file = os.path.join(save_dir, "best_model", "last_epoch.txt")
    if os.path.exists(epoch_file):
        try:
            with open(epoch_file, "r") as f:
                start_epoch = int(f.read().strip())
            if rank == 0:
                print(f"\n🚀 [EPOCH HAFIZA AKTİF] 1. Epok daha önce başarıyla bittiği için ekran sayacı da doğrudan Epoch {start_epoch+1}'den başlatılıyor!\n")
        except Exception:
            pass
    ...
    for epoch in range(start_epoch, config.tokenizer_epochs):
        ...
        for batch_idx, (ori_batch_x, _) in enumerate(train_loader):
            ...
            chunk_size = max(1, ori_batch_x.shape[0] // min(accumulation_steps, ori_batch_x.shape[0]))
            actual_accum = max(1, int(np.ceil(ori_batch_x.shape[0] / chunk_size)))
            current_batch_total_loss = 0.0
            for j in range(actual_accum):
                start_idx = j * chunk_size
                end_idx = min((j + 1) * chunk_size, ori_batch_x.shape[0])
                ...
                loss_scaled = loss / actual_accum
                current_batch_total_loss += loss.item() * (batch_x.shape[0] / ori_batch_x.shape[0])
        ...
        if rank == 0:
            print(f"\n⏳ [Epoch {epoch+1}] Doğrulama (Validation) Sınavı Başladı ({len(val_loader)} Adım)...")
            
        with torch.no_grad():
            for val_idx, (ori_batch_x, _) in enumerate(val_loader):
                ori_batch_x = ori_batch_x.to(device, non_blocking=True)
                ...
                if rank == 0 and (val_idx + 1) % 500 == 0:
                    print(f"  --> [Doğrulama] Adım {val_idx+1} / {len(val_loader)} | Anlık MSE Hata: {val_loss_item.item():.4f}")
        ...
        if rank == 0:
            ...
            with open(os.path.join(save_dir, "best_model", "last_epoch.txt"), "w") as f:
                f.write(str(epoch + 1))
