...
def train_model(model, tokenizer, device, config, save_dir, logger):
    ...
    if os.path.exists(epoch_file):
        try:
            ...
            if rank == 0:
                print(f"\n🚀 [PREDICTOR HAFIZA AKTİF] Eğitim, ekranda da doğrudan Epoch {start_epoch+1}'den devam ettiriliyor!\n")
        except Exception:
            pass
    ...
    for epoch in range(start_epoch, config.basemodel_epochs):
        ...
        if rank == 0:
            print(f"\n⏳ [Epoch {epoch+1}] Tahminci (Predictor) Doğrulama Sınavı Başladı ({len(val_loader)} Adım)...")
            
        with torch.no_grad():
            for val_idx, (batch_x, batch_x_stamp) in enumerate(val_loader):
                batch_x = batch_x.to(device, non_blocking=True)
                batch_x_stamp = batch_x_stamp.to(device, non_blocking=True)
                ...
                if rank == 0 and (val_idx + 1) % 500 == 0:
                    print(f"  --> [Doğrulama] Adım {val_idx+1} / {len(val_loader)} | Anlık Hata: {loss.item():.4f}")
